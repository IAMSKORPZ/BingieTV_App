...
class CategoryDetailController extends ChangeNotifier {
  ...
  Future<void> loadContent() async {
    try {
      _setLoading(true);
      _page = 0;
      final result = await _contentService.fetchContentPageByCategory(
        category,
        page: _page,
      );
      _contentItems = result.items;
  ...
  Future<void> loadNextPage() async {
    if (_isLoading || _isLoadingMore || !_hasNextPage || _isSearching) return;
    ...
    try {
      ...
      _page = nextPage;
      _contentItems = [..._contentItems, ...result.items];
      _hasNextPage = result.hasNextPage;
      _genres = _extractGenres(_contentItems);
    } catch (error) {
